...
class AudiobookshelfMultiSyncService:
    ...
    async def sync_all_instances(self, full_sync: bool = False) -> List[SyncResult]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                logger.error(
                    f"Erreur lors de la synchronisation de {instance.name}: {result}"
                )
            else:
        ...
        logger.info(
            f"Synchronisation multi-instances terminée: {len(sync_results)} instances traitées"
        )
        return sync_results

    async def _sync_instance_smart(
        self, instance: AudiobookshelfInstance, full_sync: bool
    ) -> SyncResult:
        """
        ...
        try:
            ...
            if not sync_service.set_instance(instance.id):
                raise Exception(
                    f"Impossible d'initialiser le client pour {instance.name}"
                )
    ...
    async def _resolve_cross_instance_conflicts(
        self, instances: List[AudiobookshelfInstance]
    ):
        """
        ...
        logger.info(
            f"Résolution des conflits terminée: {conflicts_found} détectés, {conflicts_resolved} résolus"
        )
    ...
    def _detect_conflicts(
        self, audiobook_data: Dict, instances: List[AudiobookshelfInstance]
    ) -> List[ConflictData]:
        """
        ...
        for conflict in conflicts:
            try:
                if self.conflict_resolution_strategy == "timestamp":
                    ...
                    winner_instance, winner_value = await self._resolve_by_priority(
                        conflict
                    )
                else:
                ...
                await self._apply_conflict_resolution(
                    conflict, winner_instance, winner_value
                )
                resolved_count += 1

                logger.info(
                    f"Conflit résolu pour {conflict.audiobook_id}.{conflict.field}: "
                    f"instance {winner_instance} gagnante"
                )

            except Exception as e:
                logger.error(
                    f"Erreur lors de la résolution du conflit {conflict.audiobook_id}: {str(e)}"
                )
    ...
    async def _resolve_by_priority(self, conflict: ConflictData) -> Tuple[int, any]:
        ...
        if (
            instance_a.priority <= instance_b.priority
        ):  # Priorité plus basse = plus haute priorité
    ...
    async def _apply_conflict_resolution(
        self, conflict: ConflictData, winner_instance: int, winner_value: any
    ):
        """
    ...
    def get_sync_status(self) -> Dict:
        ...
        return {
            ...
            "average_health_score": sum(i.get_health_score() for i in instances)
