...
from app.db.models.base import AuditLog, Permission, Role, User, UserCreate, UserUpdate
...
class UserService:
    ...
    async def update_user_profile(
        ...
        if updated_by and updated_by != user_id:
            ...
            if not (
                updater
                and (updater.is_superuser or has_permission(updater, "users.update"))
            ):
    ...
    async def delete_user(self, user_id: int, deleted_by: int) -> bool:
        ...
        if not (
            deleter
            and (deleter.is_superuser or has_permission(deleter, "users.delete"))
        ):
            raise PermissionError("Permission denied")
    ...
    async def assign_roles(
        self, user_id: int, role_ids: List[int], assigned_by: int
    ) -> Optional[User]:
        """
        ...
        if not (
            assigner
            and (
                assigner.is_superuser or has_permission(assigner, "users.assign_roles")
            )
        ):
            raise PermissionError("Permission denied")
    ...
    async def change_password(
        self, user_id: int, new_password: str, changed_by: Optional[int] = None
    ) -> bool:
        """
        ...
        if changed_by and changed_by != user_id:
            ...
            if not (
                changer
                and (
                    changer.is_superuser
                    or has_permission(changer, "users.change_password")
                )
            ):
                raise PermissionError("Permission denied")
