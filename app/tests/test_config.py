...
class _TestSettings(BaseSettings):
    ...
    DB_NAME: str = "testdb"
    DATABASE_URI: str = (
        "sqlite+aiosqlite:///:memory:"  # Utilisation de SQLite en mémoire pour les tests
    )
...
def apply_test_settings() -> None:
    ...
    assert (
        "sqlite" in test_settings.DATABASE_URI
    ), "La base de données de test doit utiliser SQLite"
