...
async def test_login_invalid_username(async_client: TestClient) -> None:
    ...
    data = response.json()
    assert "detail" in data, "Error detail should be in response"
    assert any(msg in data["detail"].lower() for msg in ["incorrect", "invalid", "wrong"]), \
...
async def test_test_token(async_client: TestClient, normal_user_token_headers: Dict[str, str]) -> None:
    """
