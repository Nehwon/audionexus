...
from .test_config import apply_test_settings, test_settings
...
from app.db import AsyncSessionLocal, get_async_db
...
async def async_engine() -> AsyncGenerator[AsyncEngine, None]:
    ...
    from app.db.session_manager import AsyncSessionLocal as GlobalAsyncSessionLocal
...
async def override_dependencies(db_session: AsyncSession, app: FastAPI):
    ...
    async def override_async_db():
        ...
        logger.debug(
            "Utilisation de la session de test asynchrone dans override_async_db"
        )
        try:
    ...
    async def override_async_db_session():
        ...
        logger.debug(
            "Utilisation de la session de test asynchrone dans override_async_db_session"
        )
        try:
            # S'assurer que la session est valide
            if db_session.in_transaction():
            ...
            logger.error(
                f"Erreur dans override_async_db_session: {str(e)}", exc_info=True
            )
            if db_session.in_transaction():
                await db_session.rollback()
            raise e
    ...
    from app.db.session_manager import async_engine as db_async_engine
    from app.db.session_manager import (
        init_async_engine,
    )
    ...
    if db_async_engine is None:
        logger.info(
            f"Initialisation du moteur asynchrone pour les tests avec l'URL: {test_db_url}"
        )
    ...
    logger.info(
        f"Fonction de dépendance originale (importée depuis app.core.deps): {id(original_get_async_db_session)}"
    )
    logger.info(
        f"Fonction de remplacement (créée dans conftest): {id(override_async_db_session)}"
    )
    ...
    if overridden_func:
        logger.info(
            f"Fonction de dépendance surchargée dans app.dependency_overrides: {id(overridden_func)}"
        )
    else:
        logger.warning(
            "La fonction 'get_async_db_session' n'a pas été trouvée dans app.dependency_overrides"
        )

    # Vérifier que la fonction de dépendance est bien celle attendue
    logger.info(
        f"Fonction de dépendance dans app.dependency_overrides: {app.dependency_overrides.get('get_async_db_session')}"
    )
    logger.info(
        f"Fonction de dépendance dans app.dependency_overrides (id): {id(app.dependency_overrides.get('get_async_db_session'))}"
    )

    # Afficher toutes les dépendances actuellement surchargées
    logger.info(
        f"Dépendances actuellement surchargées: {app.dependency_overrides.keys()}"
    )
