...
class UserManager:
    ...
    def create_user(self, user_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"Utilisateur créé avec succès: {user_data['username']} (ID: {user_id})"
            )
            return user_id

        except Exception as e:
            logger.error(
                f"Échec de la création de l'utilisateur {user_data.get('username')}: {str(e)}"
            )
            return None
    ...
    def update_user(self, user_id: str, user_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Échec de la mise à jour de l'utilisateur {user_id}: {str(e)}"
            )
            return False
    ...
    def delete_user(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Échec de la suppression de l'utilisateur {user_id}: {str(e)}"
            )
            return False
    ...
    def set_user_password(
        ...
        try:
            ...
            logger.error(
                f"Échec de la définition du mot de passe pour l'utilisateur {user_id}: {str(e)}"
            )
            return False
    ...
    def get_user_roles(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Échec de la récupération des rôles pour l'utilisateur {user_id}: {str(e)}"
            )
            return []
    ...
    def add_user_roles(self, user_id: str, roles: List[str]) -> bool:
        ...
        try:
            ...
            if roles_to_add:
                ...
                logger.info(
                    f"Rôles ajoutés à l'utilisateur {user_id}: {', '.join(roles)}"
                )
            return True
        except Exception as e:
            logger.error(
                f"Échec de l'ajout des rôles à l'utilisateur {user_id}: {str(e)}"
            )
            return False
    ...
    def remove_user_roles(self, user_id: str, roles: List[str]) -> bool:
        ...
        try:
            ...
            if roles_to_remove:
                self._keycloak_admin.delete_realm_roles_of_user(
                    user_id, roles_to_remove
                )
                logger.info(
                    f"Rôles supprimés de l'utilisateur {user_id}: {', '.join(roles)}"
                )
            return True
        except Exception as e:
            logger.error(
                f"Échec de la suppression des rôles de l'utilisateur {user_id}: {str(e)}"
            )
            return False
