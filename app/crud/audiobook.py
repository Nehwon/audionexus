...
from app.schemas.audiobook import (
    AudiobookCreate, AudiobookUpdate, AudiobookInDB,
    AudiobookQuery, AudiobookProgressCreate, AudiobookProgressUpdate
)
...
class CRUDAudiobook:
    ...
    @staticmethod
    def get_by_external_id(db: Session, external_id: str, source: str = "audiobookshelf") -> Optional[Audiobook]:
        """Récupère un livre audio par son ID externe et sa source."""
        return db.query(Audiobook).filter(
            Audiobook.external_id == external_id,
            Audiobook.source == source
        ).first()
        ...
        *,
        ...
        if query:
            ...
            if sort_field is not None:
                sort_expr = sort_field.asc() if query.order == "asc" else sort_field.desc()
    ...
    @staticmethod
    def create(db: Session, obj_in: Union[AudiobookCreate, Dict[str, Any]]) -> Audiobook:
...
class CRUDAudiobookProgress:
    ...
    def get_progress(
        ...
        return db.query(AudiobookProgress).filter(
    ...
    def get_user_progress(
        ...
        *,
        ...
        q = db.query(AudiobookProgress).filter(
            AudiobookProgress.user_id == user_id
        )
    ...
    def create_progress(
        db: Session,
        *,
        obj_in: Union[AudiobookProgressCreate, Dict[str, Any]]
    ) -> AudiobookProgress:
    ...
    def get_recently_played(
        ...
        *,
        ...
        return db.query(AudiobookProgress).filter(
            ...
            AudiobookProgress.last_played.desc()
        ).limit(limit).all()
    ...
    def get_in_progress(
        ...
        *,
        ...
        return db.query(AudiobookProgress).filter(
            ...
            AudiobookProgress.updated_at.desc()
        ).limit(limit).all()
