...
from app.db.models.base import Token, UserCreate, UserInDB
...
def get_csrf_token() -> str:
    ...
    def _verify_csrf_token(csrf_token: str = Header(None, alias="X-CSRF-Token")):
        ...
        if csrf_token not in csrf_tokens:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Token CSRF invalide"
            )
...
async def register(
    request: Request, user_data: UserCreate, csrf_token: str = Depends(get_csrf_token())
):
