...
user_model = api.model('User', {
    ...
    'is_superuser': fields.Boolean(description='Indique si l\'utilisateur est administrateur')
})

token_model = api.model('Token', {
    'access_token': fields.String(required=True, description='JWT d\'accès'),
    'token_type': fields.String(required=True, default='bearer', description='Type de token')
})
...
register_parser.add_argument('username', type=str, required=True, help='Nom d\'utilisateur')
...
def _create_user(username: str, email: str, password: str, full_name: str = None) -> User:
...
reset_password_parser.add_argument('email', type=str, required=True, help='Adresse email de l\'utilisateur')

new_password_parser = reqparse.RequestParser()
new_password_parser.add_argument('token', type=str, required=True, help='Jeton de réinitialisation')
new_password_parser.add_argument('new_password', type=str, required=True, help='Nouveau mot de passe')
...
class ForgotPassword(Resource):
    ...
    def post(self):
        ...
        if user and user.is_active:
            ...
            current_app.logger.info("Envoi d'email de réinitialisation désactivé (fonctionnalité non prioritaire)")
