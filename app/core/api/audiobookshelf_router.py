...
async def get_recently_added(
    limit: int = Query(
        10, ge=1, le=100, description="Nombre maximum de livres à retourner"
    ),
    client: AudiobookshelfClient = Depends(get_audiobookshelf_client),
    current_user: User = Depends(get_current_user),
):
...
async def search_audiobooks(
    ...
    library_id: Optional[str] = Query(
        None, description="Filtrer par ID de bibliothèque"
    ),
    limit: int = Query(20, ge=1, le=100, description="Nombre maximum de résultats"),
    offset: int = Query(0, ge=0, description="Décalage pour la pagination"),
    sort_by: str = Query(
        "addedAt", description="Critère de tri (addedAt, title, author, duration)"
    ),
...
async def filter_audiobooks(
    ...
    min_duration: Optional[int] = Query(
        None, ge=0, description="Durée minimum en secondes"
    ),
    library_id: Optional[str] = Query(
        None, description="Filtrer par ID de bibliothèque"
    ),
            ...
            try:
                ...
                for item in items[:200]:  # Limite pour éviter les timeouts
                    ...
                    if (
                        author
                        and author.lower()
                        not in " ".join(metadata.get("author", [])).lower()
                    ):
                        continue
                    if genre and not any(
                        genre.lower() in g.lower() for g in metadata.get("genres", [])
                    ):
                        continue
                    if series and not any(
                        series.lower() in s.get("name", "").lower()
                        for s in metadata.get("series", [])
                    ):
                        continue
                    if (
                        language
                        and language.lower() != metadata.get("language", "").lower()
                    ):
                        continue
                    ...
                    if (
                        min_duration
                        and (metadata.get("duration", 0) or 0) < min_duration
                    ):
                        continue
...
async def get_reading_progress(
    ...
    try:
        ...
        _handle_abs_error(
            e,
            f"la récupération de la progression pour l'utilisateur {user_id}, livre {item_id}",
        )
...
async def update_reading_progress(
    ...
    try:
        ...
        _handle_abs_error(
            e,
            f"la mise à jour de la progression pour l'utilisateur {user_id}, livre {item_id}",
        )
