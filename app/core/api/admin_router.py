...
from app.db.models import User, Audiobook, AudiobookProgress, Book, Library
...
async def get_admin_metrics(
    ...
    finished_books = db.query(func.count(AudiobookProgress.id)).filter(AudiobookProgress.is_finished == True).scalar()
    ...
    unique_libraries_audiobookshelf = db.query(func.count(func.distinct(Audiobook.library_id))).scalar()
    unique_libraries_local = db.query(func.count(func.distinct(Library.id))).scalar()
    total_libraries = max(unique_libraries_audiobookshelf, unique_libraries_local)  # Approximation

    # Utilisation récente (derniers 30 jours)
    thirty_days_ago = datetime.utcnow() - timedelta(days=30)
    recent_users = db.query(func.count(User.id)).filter(User.last_seen >= thirty_days_ago).scalar()
    recent_progress = db.query(func.count(AudiobookProgress.id)).filter(AudiobookProgress.updated_at >= thirty_days_ago).scalar()
...
async def get_trends(
    ...
    for i in range(days):
        ...
        daily_users = db.query(func.count(User.id)).filter(User.created_at <= next_date).scalar()
        daily_books = db.query(func.count(Book.id)).filter(Book.created_at <= next_date).scalar() + \
                     db.query(func.count(Audiobook.id)).filter(Audiobook.created_at <= next_date).scalar()
