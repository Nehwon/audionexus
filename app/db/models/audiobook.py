...
class Audiobook(Base):
    ...
    series = Column(
        JSON, default=list
    )  # Liste de dicts: [{"name": str, "sequence": float}]
    ...
    )  # Chemin vers le fichier audio principal

    # Relations
    library_id = Column(
        String(100), nullable=False, index=True
    )  # ID de la bibliothèque dans Audiobookshelf
    ...
    )  # Dernière synchronisation avec Audiobookshelf

    # Relations
    progress = relationship(
        "AudiobookProgress", back_populates="audiobook", cascade="all, delete-orphan"
    )
...
class AudiobookProgress(Base):
    ...
    audiobook_id = Column(
        Integer, ForeignKey("audiobooks.id"), nullable=False, index=True
    )
